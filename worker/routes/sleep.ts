...
const sleep = new Hono<AppBindings>();

sleep.post("/start", async (c) => {
  const repo = createD1SleepRepository(c.env);

  const active = await repo.getActive();
  if (active) {
    return c.json({ error: "A sleep timer is already running" }, 409);
  }
...
sleep.post("/:id/stop", zValidator("json", stopSchema), async (c) => {
  const id = parseInt(c.req.param("id"));
...
sleep.put("/:id", zValidator("json", editSchema), async (c) => {
  const id = parseInt(c.req.param("id"));
  const body = c.req.valid("json");
  const repo = createD1SleepRepository(c.env);
